...
from bes.nn.branched_policy import BranchedMLP
...
def run_with_ray(model_cls, env_name, population_size, num_cma_iterations, tempdir, model_kwargs={},
                  limit_steps_fn=lambda iteration: np.inf):
    
    model_factory = functools.partial(model_cls, env.observation_space.shape[0], env.action_space.shape[0],
                                      **model_kwargs)
    ...
    for cma_iteration in range(1, num_cma_iterations+1):
        es_popsize = 50
        candidates = es.ask(number=es_popsize)
...
def run_experiment(env_name, population_size, num_cma_iterations, n_workers, step_scaling, min_steps, max_steps,
    ...
    run_info = dict(
        model_class_name=str(BranchedMLP),
